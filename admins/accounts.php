...
<!--ACCOUNTS TABS-->
	...
	<!--ACCOUNTS TRANSACTIONS SECTION-->
				<div id="transactions">
					<a href="pdfs/transaction_pdf.php?hst=<?php echo $_SESSION['hostel']?>" target="_blank"><input type="button" value="Print" class="btn btn-warning" /></a>
						...
						<div class="modal-body">
							...
							<div class="row">
								...
								<div class="col-lg-6 col-md-6">
										<div class="form-group">
											<label>Account name</label>
											<input type="text" name="account_name" id="account_name" class="form-control" placeholder="Enter new account name" onblur="ValidateVariousCharacters('account_name',5,50,'accountnameresponse')"/>
											<div id="accountnameresponse"></div>
										</div>
									</form>
								</div>
								<div class="col">
									<input type="submit" name="add" class="btn btn-primary form-control" value="Save" />
								</div>
							</div>
							</form>
						</div>
			...
			<?php
				if(isset($_POST['add'])){
					...
					$s = "INSERT INTO account_name (acc_id,acn_name,host_id) values('$acc_type','$acc_name','$hostel')";
					$rS = mysqli_query($con, $s);
					if($rS){
				...
				<div class="modal-dialog modal-lg" >
					<div class="modal-content">
						...
						<div class="modal-body">
							...
							<div class="row">
										...
										<select name="account_title" id="account_title" class="form-control">
											...
											<?php
												...
												while($row=mysqli_fetch_array($runquery)){
													...
													$name = $row['acn_name'];
													echo"
														<option value='$acn_id'>$name</option>
													";
												}
											?>
										</select>
								...
								<div class="col-lg-6 col-md-6">
										<div class="form-group">
											...
											<input type="text" name="description" onkeyup="ValidateVariousCharacters('trans_desc',5,100,'transdescresponse')" id="trans_desc" class="form-control" placeholder="Enter some description" />
											<div id="transdescresponse"></div>
										</div>
								</div>
							</div>
							<div class="row">
								<div class="col-lg-6 col-md-6">
									<label>Amount</label>
									<input type="text" name="amount" id="trans_amount" class="form-control" placeholder="Enter amount" onkeyup="ValidateNumerals('trans_amount',2,6,'transamountresponse')"/>
									<div id="transamountresponse"></div>
								</div>
								<!--<div class="col-lg-6 col-md-6">
									<div class="form-group">
										<label></label>
							...
							<input type="submit" name="transact" id="submitTransaction" class="btn btn-primary" value="Save" />
							</form>
						</div>
						<div class="modal-footer">
							<button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
			...
			<?php
				if(isset($_POST['transact'])){
					...
					$s = "INSERT INTO transactions (acn_id,Description,amount,tra_date) values('$acc_name','$description','$amount',now())";
					$rS = mysqli_query($con, $s);
					if($rS){
