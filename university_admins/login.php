<?php
	...
	if(isset($_POST['login'])){
		...
		$pass = md5($password);
		$q = "SELECT * FROM university_admins WHERE admin_email='".$email."' AND admin_password='".$pass."'";
		$rQ = mysqli_query($con, $q);
		$row = mysqli_fetch_array($rQ);
		if(mysqli_num_rows($rQ) > 0){
				$_SESSION['university'] = $row['uni_id'];
...
<html>
	<head>
		<title>HOSTELS ONLINE BOOKING SYSTEM</title>
