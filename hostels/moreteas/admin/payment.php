		...
		if($count > 0){
			...
			if($bal > 0){
				...
				$runSql = mysqli_query($con, $sql);
					if($runSql){
						echo "<script>alert('You have successfully made a complete payment of your rent payment')</script>";
						echo "<script>window.open('admin.php?payment','_self')</script>";
					}else{
						echo "<script>alert('There was a problem entering the payment.Please try again!')</script>";
					}
								...
								while($row = mysqli_fetch_array($runQ)){
									...
									echo"
										<tr>
											...
											<th>$amount</th>
											<th>$date</th>
											<th><input type='submit' class='btn btn-primary btn-sm pedit' id='$id' value='Edit'/></th>
										</tr>
									";
								}
							?>
