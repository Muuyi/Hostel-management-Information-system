							...
							echo '
								<tr>
									...
									if($passport == ''){
										...
										echo '<img src="images/'.$passport.'" width="50px" height="50px" />';
									}
							echo 	'</th>
									<th>'.$idNo.'</th>
									<th>'.$email.'</th>
...
<?php
	if(isset($_POST['save'])){
		...
		$user = $_POST['sys_user'];
		move_uploaded_file($profile,"images/$pic");
		$status = "active";
		//$pic = addslashes(file_get_contents($profile));
		$q = "INSERT INTO admins (admin_FName,admin_LName, admin_Email,admin_IDNo,admin_Username,admin_pass,admin_pic,status,user_type) VALUES('$fname','$lname','$email','$id','$username','$password','$pic','$status','$user')"; 
		$rQ = mysqli_query($con, $q);
		if($rQ){
