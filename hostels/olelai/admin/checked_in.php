...
<table width="100%" class="cTable" border="1px solid #000000">
	...
	<?php
		$i=0;
		$get_client = "select * from clients WHERE status = 1";
	...
	<tr>
		...
		<td>
			<?php echo "$c_name" ?><br />
<?php echo "<a href='admin.php?details=$cId'><button class='button'>More Details</button></a>" ?>
		</td>
		...
		<td>
			<?php 
			//Clients receipt printing
			echo "<form action='receipt.php?clientId=$cId' method='POST' enctype='multipart/form-data'>
				<input type='submit' name='print' value='print' class='check' />
			</form>"; ?>
		</td>
		<td><a href="admin.php?edit_client=<?php echo $cId ?>">Edit</a></td>
		<td><a href="admin.php?delete_client=<?php echo $cId ?>">Delete</a></td>
	</tr>
		<?php } ?>
</table>
	<?php } ?>